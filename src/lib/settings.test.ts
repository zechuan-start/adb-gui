...
describe("settings migration", () => {
  ...
  it("refuses a newer version instead of guessing its fields", () => {
    ...
    ).toThrow("settings.unsupportedVersion");
    expect(() =>
      decodeSettings(
  ...
  it.each([undefined, null, "1", 1.5, Number.NaN])(
    ...
    (version) => {
      expect(() => migrateSettings(version, {})).toThrow("settings.unsupportedVersion");
    },
  );
  ...
  it("keeps rejecting a malformed settings body after the version check", () => {
    ...
    expect(() => migrateSettings(SETTINGS_VERSION, [])).toThrow("settings.invalidFormat");
  });
});
