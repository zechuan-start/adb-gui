...
- Report batch parsing failures through `AppError`: `files.metadataNotUtf8`, `files.invalidBatchMetadata`, `files.missingBatchPath`, and `files.invalidMode`. Keep their frontend schemas and both translations synchronized; preserve raw numeric parse diagnostics in `detail`.
