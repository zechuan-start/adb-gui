...
2. `app-info.dex` 已用 `scripts/build-app-info-dex/build.sh` 重建并提交。
   ...
   但**一点都不会变快**，等于本任务白做。
3. `09-03-device-transport-merge` 的 Rust 身份字段已在当前集成分支完成并通过验证：
...
5. 只有图标走"按需请求"：本次结果中 `(pkg, lastUpdateTime)` 未命中缓存的包，
   ...
   `lastUpdateTime <= 0` 表示失效键不可信，这类应用必须**每轮都进入图标请求集、
   但永远不写入持久化缓存**；不能把它从差集里跳过，否则成功路径不会再触发
   图标兜底，界面会一直显示占位图标。
...
7. 元数据落盘一份，用于选中设备时的**乐观首屏**：立刻画出上次的列表，
   ...
   不能出现"缓存赢了新数据"的情况。若缓存首屏已显示、随后实时元数据读取失败，
   保留完整缓存数据并显示降级黄条，明确标注数据来自上次读取；不要替换成裸包名。
...
9. 缓存不可用（目录建不了、文件损坏、JSON 解析失败）时一律**静默退化**成
   ...
   读写命令的函数体保持同步，但必须用 `#[tauri::command(async)]` 包装，
   将批量 PNG 文件 I/O 移出 Tauri 主线程；前端调用签名仍然是 Promise，不变。
...
13. 同一台设备**同时**通过 USB 与 WiFi 连接时会有两条不同 serial，
    ...
    锁用 `std::sync::Mutex`（缓存命令的函数体是同步的、只做文件 I/O，不跨 await；
    命令入口仍按 Requirement 9 标成 `#[tauri::command(async)]`）。
...
- [ ] 同一台设备第二次打开应用面板，若期间没有装卸/更新应用且所有条目的
      `lastUpdateTime > 0`，
      **完全不发起图标请求**，图标直接来自缓存。
- [ ] 装一个新应用 → 只有这一个包进过滤集；卸载一个 → 缓存条目被删；
      更新一个（`lastUpdateTime` 变化）→ 只有这一个包被重新请求。
      三种情况各有一条自动化测试覆盖 diff 纯函数。
- [ ] `lastUpdateTime <= 0` 的应用每轮都会请求实时图标，但不会进入 `index.json`；
      该规则有自动化测试，不能出现永久占位图标。
- [ ] 元数据每次都是新鲜的：改了应用版本后，即使图标命中缓存，
      列表里的版本号/APK 大小也必须立刻更新。
- [ ] 乐观首屏不会覆盖新数据：阶段 1 返回后列表以新数据为准。
- [ ] 乐观首屏已显示时若阶段 1 失败，保留缓存列表并显示“上次读取”黄条；
      没有可用缓存时才沿用父任务的裸包名降级路径。
- [ ] 缓存目录被删除 / 文件被写坏 / JSON 非法时，列表仍能正常加载
      （静默退化为全量取图标），不弹错、不出黄条。
- [ ] 只更新一个包的图标时，旧 index 中其他 `(packageName, lastUpdateTime)` 仍匹配的
      图标引用必须原样保留；连续写第二次且 `newIcons` 为空时不能把已有图标剪掉。
- [ ] `index.json` 已存在时可在 macOS、Linux、Windows 上重复替换；Unix 使用同目录
      原子 rename，Windows 使用明确的 remove-then-rename 缓存降级路径，不依赖
      `fs::rename` 覆盖已存在目标这一跨平台不成立的假设。
...
- [x] 乐观首屏后实时刷新失败怎么办？→ **保留缓存并显示黄条**，明确数据来自
      上次读取。缓存可能过期，但完整的名称、版本和图标比主动退化成裸包名更有用；
      没有缓存时仍走父任务原有降级路径。
...
- 本任务是纯增量的 Rust + 前端改动，**不碰 `Main.java`、不需要重建 dex**，
  可以独立验证、独立回滚。
- 执行分支：当前集成分支 `claude/app-info-read-stability-tj25vt`，在设备传输合并
  完成并通过验证后串行实施。
