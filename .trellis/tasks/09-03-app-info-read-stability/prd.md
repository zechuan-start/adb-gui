...
4. 批量图标调用失败/超时不得影响已渲染的元数据，静默退到既有的按需
   `get_app_icon` 懒加载路径。
5. 批量图标命令支持**可选的包名过滤**：不传过滤则返回全部图标（冷启动路径）；
   传了则只渲染指定包名，且按固定上限分批调用。过滤列表以命令行参数传递，
   包名在拼进 shell 命令前必须校验字符集。
   —— 这一条本轮没有调用方会传非空过滤，它是为子任务
   `09-03-app-info-cache`（按 `(packageName, lastUpdateTime)` 缓存图标）预留的契约。
   放在本轮做的唯一理由：改这条契约要重建 `app-info.dex`，而重建 dex 依赖用户
   本地的 Android SDK 环境，两次重建并成一次。
...
> 一台设备可以**同时**通过 USB 和 Wi-Fi 连接，`adb devices` 会列出两条不同 serial
...
> 于是双双 push，后者截断前者正在执行的 dex → dex verify 崩溃 → 按 Requirement 14
> 属于非零退出，不重试，直接弹黄条。这正是本条要修的 bug 换个入口重现。
>
> 全局锁的代价接近零：面板永远只加载当前选中的那一台设备，本就不存在多设备并发加载。
...
13. 元数据与图标两种模式使用各自的超时预算。
14. **超时**或 **push 失败**时自动重试一次（重试前强制重新 push）；
    `app_process` 明确非零退出（ROM 不兼容）**不重试**，直接返回错误。

### A5 — 失败原因可见 + 可重试

15. 降级黄条展示具体失败原因（可折叠，过长时截断展示但保留完整文本可复制）。
16. 黄条提供「重试」按钮，重新走完整的 `get_installed_apps` 路径。

### C9 — Context 引导多级回退

17. `createSystemContext()` 增加第二条轻量路径：`ActivityThread` 的无参构造 +
    `getSystemContext()`（跳过 `attach`/Instrumentation/Application.onCreate）。
18. `systemMain()` 保持**第一优先**，轻量路径作为其失败后的回退——本轮不调换顺序，
    确保对现在能成功的设备零回归。
19. 每一级失败都把完整 stack trace 写入 stderr，使 A5 的「详情」真正可用于诊断。
...
- **B8 图标本地缓存**：已拆为子任务 `09-03-app-info-cache`，本轮只交付它需要的
  包名过滤契约（Requirement 5），不做任何持久化。缓存的失效键、设备键、
...
- [ ] 批量图标命令接受可选包名过滤：空过滤 → 全部图标（本轮唯一实际调用方式）；
      非空过滤 → 只渲染指定包名并按上限分批。包名字符集校验有单元测试覆盖。
      旧 dex 忽略过滤参数返回全集时，调用方必须能正常处理这个超集而不报错。
